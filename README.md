...
```
├── config/
│   └── sys_msg.txt         # System prompt for Gemma
├── src/
│   └── main.rs             # Rust terminal UI
├── tools/
│   ├── nmap.py             # Nmap tool
│   ├── searchsploit.py     # SearchSploit tool
│   ├── gemma.py            # Main agent implementation
...
1. **Download Gemma model**
   ...
   Gemma is gated on Hugging Face. Accept access to `google/gemma-2-2b`, create a read token, then run:
...
* Gemma’s Flask API will be available on `http://localhost:5000`.
* **Environment Variables**:

  * `HF_TOKEN` - Hugging Face access token used to download and run Gemma.
